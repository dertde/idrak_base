...
$GET_product_del = $_GET['product_del'];

$GET_code_client = $_GET['code_client'];

if ($_GET['client_id']) {
    $code_client = $_GET['client_id'];
}
if ($_GET['code_client']) {
    $code_client = $_GET['code_client'];
}
if ($_GET['hiddenField']) {
    $code_client = $_GET['hiddenField'];
}
...
if ($GET_code_client <> '') {
    ...
    $sql = "DELETE FROM ".$c_alfa."t_sklad_temp";
    $result = mysql_query($sql) or die("2Could not connect: " . mysql_error());
}

//	echo '-----'.$GET_product_del;
...
if ($GET_product_del <> '') {
    $sql = "DELETE FROM ".$c_alfa."t_sklad_temp WHERE c_id_product =" . $GET_product_del . "";
...
<img src="blank.gif" width="10" height="25">
  ...
  <input name="hiddenField" type="hidden" value="<?php echo ($code_client); ?>"  />
  <input type="submit" name="Submit" value="Əlavə et Товар" />
</form>
</td>
</tr>
</table>
...
<table border="0" cellpadding="0" cellspacing="0" >
 ...
 <td class="tabhead" align='left'><img src="blank.gif" width="85" height="6"><br><b>Başlıq</b></td>
...
<?php
$sql = "SELECT * FROM ".$c_alfa."t_sklad_temp WHERE 1 LIMIT 0, 30 ";
...
while ($row = mysql_fetch_assoc($result)) {
	...
	echo "<td class='tabval' align='right'><a href='qw2.php?client_id=" . $code_client .
        "&product_del=" . $c_id_val . "'>Удалить</a></td>";
...
<img src="blank.gif" width="10" height="25"> Ввидите код клиента  
<input name="code_client" type="text"  value="<?php echo ($code_client);?>" size="5" />
...
<?php
echo '<a href="client_zakazi_777.php?client_id= ' . $code_client  .
    '">Заказы</a>';
?></p>
</body>
</html>
